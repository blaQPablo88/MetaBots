#property strict
#property copyright "Lulus Arga Kustyarso"
#property link "Trickerless 7.22"  
int MAGIC = 20130721; 
extern string TOOLS = ".............................................................................................................";  
...
void preparePositions() {
   ...
   for( int i = 0 ; i < OrdersTotal(); i++ ) {
      ...
      if( OrderSymbol() == Symbol() && OrderComment() == "Trickerless 7.22 Backup" ) totalBackupTrades = totalBackupTrades + 1;
   ...
   if( ( SafeSpread && spread < MaxSpread ) || !SafeSpread ){
      if( !nearLongPosition && bullish && sellLots == 0 ) {
        ...
        if( basketCount < MaxTrades ){
           ...
           OrderSend( Symbol(), OP_BUY , lotSize, Ask, slippage, 0, 0, "Trickerless 7.22 Min " + DoubleToStr( basketNumber, 0 ), MAGIC ) ;
           lastTradeTime = TimeCurrent(); 
         ...
         if( basketCount < MaxTrades ){
            ...
            OrderSend( Symbol(), OP_SELL, lotSize, Bid, slippage, 0, 0, "Trickerless 7.22 Min " + DoubleToStr( basketNumber, 0 ), MAGIC ) ; 
            basketCount = basketCount + 1; 
...
void sendBack(){
   if( ( ContinueTrading || ( !ContinueTrading && totalBackupTrades > 0 ) ) && ( totalBackupTrades < MaxTrades ) ) {
      if( MathAbs( High[0] - Low[0] ) > CandleSpike * MathAbs(High[1] - Low[1] ) && Open[0] < Close[0] && Close[0] < ( High[0] + Low[0] ) / 2 && ( ( !AllowHedge && sellLots == 0 ) || AllowHedge ) ){
         ...
         OrderSend( Symbol(), OP_BUY , backupLotSize, Ask, slippage, 0, 0, "Trickerless 7.22 Backup " + DoubleToStr( basketNumber, 0 ), MAGIC ) ; 
         lastTradeTime = TimeCurrent();
      } 
   ...
   if( ( ContinueTrading || ( !ContinueTrading && totalTrades > 0 ) ) && ( totalTrades < MaxTrades )  ) { 
      if( !nearLongPosition  && sellLots == 0 ) {
         if( MathAbs( High[0] - Low[0] ) > CandleSpike * MathAbs( High[1] - Low[1] ) && Open[0] < Close[0] && Close[0] < ( High[0] + Low[0] ) / 2 ){ 
            ...
            OrderSend( Symbol(), OP_BUY , newsLotSize, Ask, slippage, 0, 0, "Trickerless 7.22 News " + DoubleToStr( basketNumber, 0 ), MAGIC ) ; 
            lastTradeTime = TimeCurrent();
         } 
      }
